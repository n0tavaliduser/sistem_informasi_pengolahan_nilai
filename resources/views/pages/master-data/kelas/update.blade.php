...
<div class="row">
    <div class="col-xl-12">
        ...
        <div class="card">
            ...
            <form method="post" action="{{ route('master-data.kelas.update', $kelas) }}">
                @csrf
                @method('patch')
                <div class="card-body">
                    <div class="row">
                        <div class="form-group mb-3 col-6">
                            ...
                            <input type="text" name="nama_kelas" id="nama_kelas" value="{{ $kelas->nama_kelas }}" class="form-control {{ !$errors->has('nama_kelas')?:'is-invalid' }}" placeholder="Nama Kelas" required>
                            @error('nama_kelas')
                            <small>{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3 col-6">
                            ...
                            <input type="number" name="tingkat" id="tingkat" value="{{ $kelas->tingkat }}" class="form-control {{ !$errors->has('tingkat')?:'is-invalid' }}" placeholder="Tingkat" required>
                            @error('tingkat')
                            <small>{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="guru_id" id="guru_id" class="form-control {{ !$errors->has('guru_id')?:'is-invalid' }}">
                                ...
                                @foreach ($semua_guru as $guru)
                                    <option value="{{ $guru->id }}" {{ $kelas->guru_id === $guru->id ? 'selected' : '' }}>{{ $guru->nama_lengkap }}</option>
                                @endforeach
                            </select>
                            @error('guru_id')
                            <small>{{ $message }}</small>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="jurusan_id" id="jurusan_id" class="form-control {{ !$errors->has('jurusan_id')?:'is-invalid' }}">
                                ...
                                @foreach ($semua_jurusan as $jurusan)
                                    <option value="{{ $jurusan->id }}" {{ $kelas->jurusan_id === $jurusan->id ? 'selected' : '' }}>{{ $jurusan->nama_jurusan }}</option>
                                @endforeach
                            </select>
                            @error('jurusan_id')
                            <small>{{ $message }}</small>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="tahun_ajaran_id" id="tahun_ajaran_id" class="form-control {{ !$errors->has('tahun_ajaran_id')?:'is-invalid' }}">
                                ...
                                <option value="{{ $tahun_ajaran->id }}" {{ $kelas->tahun_ajaran_id === $tahun_ajaran->id ? 'selected' : '' }}>{{ $tahun_ajaran->tahun_mulai }} - {{ $tahun_ajaran->tahun_berakhir }}</option>
                                @endforeach
                            </select>
